...
- (CGSize)getSizeforText:(NSString *)messageText andFont:(UIFont *)font width:(CGFloat)width;
...
- (void)showUsingAnimation:(BOOL)animated {
	...
	// Fade in
	if (animated) {
        [UIView animateWithDuration:0.3 delay:0 options:(UIViewAnimationOptions)UIViewKeyframeAnimationOptionCalculationModeLinear animations:^{
            self.alpha = 1.0f;
...
- (CGSize)getSizeforText:(NSString *)messageText andFont:(UIFont *)font width:(CGFloat)width {
    CGSize textSize;
    //CGFloat width = 200;
    NSInteger numRows = (messageText.length / [self maxCharactersPerLine]) + 1;
    
    NSInteger numberOfLines = [messageText componentsSeparatedByString:@"\n"].count + 1;
    
    CGFloat height = MAX(numRows, numberOfLines) * 35.0f;
    
    //height = height + numberOfLines*1.5;
    
    
    
    textSize = [messageText  boundingRectWithSize:CGSizeMake(width, height)
...
- (void)layoutSubviews {
    ...
    CGSize labelSize =  [self getSizeforText:label.text andFont:label.font width:200];
